...
download_needed() 
{
    ...
    for i in \
        ...
        wget https://github.com/uvzen/RVCD-bash-builder/releases/download/APPS/aapt2 \
        ; do
        n=$(($n+1))	
...
ttpatch()
{
	...
	java -jar packages/revanced-cli*.jar -a $twitter -c -o Twitter.apk -b packages/revanced-patches*.jar -m packages/app-release-unsigned.apk --custom-aapt2-binary /data/data/com.termux/files/usr/bin/aapt2
	printf '%b\n' "";
	printf '%b\n' "${CYAN}Done${NC}";
	printf '%b\n' "";
	ttname
...
rtpatch()
{
	...
	java -jar packages/revanced-cli*.jar -a $reddit -c -o Reddit.apk -b packages/revanced-patches*.jar -m packages/app-release-unsigned.apk -r --custom-aapt2-binary /data/data/com.termux/files/usr/bin/aapt2
	printf '%b\n' "";
	printf '%b\n' "${CYAN}Done${NC}";
	printf '%b\n' "";
	rtname
